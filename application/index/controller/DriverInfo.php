        ...
        if (!empty($s_time) && !empty($e_time)) {
            ...
            if (!empty($obd_id)) {
                    $rst = $merchant
                         ->table("t_car_info")
                         ->alias("c")
                         ->field("jt.id,jt.openCarId,jt.openId,jt.obdTime,jt.longitude,jt.latitude,jt.create_date,jt.modify_date,c.merchant_id,c.obd_mac,c.obd_id,jt.dataValid,jt.gpsMode,jt.accStatus,jt.gpsSpeed")
                         ->join("chegongbao.kartor_cstdp_0007 jt","c.obd_mac = jt.openCarId")
                         ->where(
                            array(
                                "obd_id"=>array("in",$obd_id),
                                "obdTime"=>array("between",[(strtotime($s_time)*1000),(strtotime($e_time)*1000)]),
                                "dataValid"=>array("eq",1),
                                "gpsMode"=>array("eq",1),
                                "latitude"=>array("neq","0"),
                                "accStatus"=>array("eq",1),
                            ))
                         ->order("obdTime")
                         ->select();
                         // $sss = $merchant->getlastsql();
                         // $this->make_json_error($sss);
            } elseif (!empty($user)) {
                    $rst = $current
        	...
        	if( $rst ){
        		...
        		foreach ($rst as $key => $value) {
        			$t = md5(strtotime($value['obdTime']).$value['openId']); // 唯一标示
        			if( !isset($ret[$t]) ){
                        // $this->make_json_error(substr($value['obdTime'],0,10));
        				$ret[$t][] = $value['longitude'].",".$value['latitude']; // 经纬度
                        $ret[$t]['start_time'] = $value['create_date']; // 起点时间
                        $ret[$t]['obd_id'] = $value['obd_id'];// 轨迹id
                        $ret[$t]['user_id'] = $value['openId']; //用户唯一id
        			} else {
        				$ret[$t][] = $value['longitude'].",".$value['latitude'];
                        $ret[$t]['end_time'] = $value['modify_date'];//结束时间 
        			}
        		}
        			// $this->make_json_error($ret);

        		$keys = array_keys($ret);            		
                foreach ($keys as $item) {
                    $ret1['obd_id'] = $ret[$item]['obd_id'];
                    $ret1['user_id'] = $ret[$item]['user_id'];
                    $ret1['start_time'] = $ret[$item]['start_time'];
                    $ret1['end_time'] = isset($ret[$item]['end_time']) ? $ret[$item]['end_time'] : $ret1['start_time'];
                    unset($ret[$item]['start_time'], $ret[$item]['end_time'], $ret[$item]['obd_id'], $ret[$item]['user_id']);
                    $ret[$item] = implode('|', $ret[$item]);
                    $ret[$item] = (array)$this->post_amaps($ret[$item]);
                    $ret[$item] = explode(';', $ret[$item]['locations']);
                    $ret[$item]['start_time'] = $ret1['start_time'];
                    $ret[$item]['end_time'] = $ret1['end_time'];
                    $ret[$item]['obd_id'] = $ret1['obd_id'];
                    $ret[$item]['user_id'] = $ret1['user_id'];
                }
        	...
        	exit(json_encode($ret));
        		...
        		exit(json_encode(array("error"=>0,"msg"=>"没有轨迹")));
        	}
        } else {
...
//        ");
        try {
            $res = Db::query("
            select c.*,t.fuel current_oil,t.mileage current_mileage,u.username,t.obdTime from (
            select a.openCarId,a.obd_id,a.plate_num,a.department_id,b.department_name,a.merchant_id from  
            chegong_merchant.t_car_info a 
            left join chegong_merchant.t_department_info b 
            on (a.department_id = b.id and a.merchant_id = b.merchant_id) where $where) c 
            left join chegongbao.kartor_cstdp_0008 t on (
            c.openCarId=t.openCarId and t.openId<>'' and t.mileage>0 and t.obdTime > $start_time-86400000 and t.obdTime < $end_time+86400000) 
            left join chegong_merchant.t_user u on (c.merchant_id=u.id) order by t.obdTime desc
        ");
        ...
        if (empty($res)) {
            ...
            foreach ($res as $item) {
                if (empty($list[$item['obd_id']])) {
                    ...
                    $list[$item['obd_id']]['current_mileage'] = 0;
                    $list[$item['obd_id']]['current_oil'] = 0;
                    $compare[$item['obd_id']]['obdTime'] = $item['obdTime'];
                    $mileage[$item['obd_id']]['current_mileage'] = $item['current_mileage'];
                    $oil[$item['obd_id']]['current_oil'] = $item['current_oil'];
                } else {
                    if ($compare[$item['obd_id']]['obdTime'] - $item['obdTime'] > 180000) {
                        if ($compare[$item['obd_id']]['obdTime'] < $end_time) {
                            if(empty($currentMaxMileage[$item['obd_id']]['current_mileage'])){
                                $currentMaxMileage[$item['obd_id']]['current_mileage'] = $mileage[$item['obd_id']]['current_mileage'];
                                $currentMaxOil[$item['obd_id']]['current_oil'] = $oil[$item['obd_id']]['current_oil'];
                            }
                            if($compare[$item['obd_id']]['obdTime'] > $start_time){
                                $list[$item['obd_id']]['current_mileage'] = $currentMaxMileage[$item['obd_id']]['current_mileage'] - $item['current_mileage'];
                                $list[$item['obd_id']]['current_oil'] = $currentMaxOil[$item['obd_id']]['current_oil'] - $item['current_oil'];
                            }
                        }else{
                            $mileage[$item['obd_id']]['current_mileage'] = $item['current_mileage'];
                            $oil[$item['obd_id']]['current_oil'] = $item['current_oil'];
                        }
//                        if ($compare[$item['obd_id']]['obdTime'] > $start_time && $compare[$item['obd_id']]['obdTime'] < $end_time) {
//                            $list[$item['obd_id']]['current_mileage'] += $mileage[$item['obd_id']]['current_mileage'] - $item['current_mileage'];
//                            $list[$item['obd_id']]['current_oil'] += $oil[$item['obd_id']]['current_oil'] - $item['current_oil'];
//                            $mileage[$item['obd_id']]['current_mileage'] = $item['current_mileage'];
//                            $oil[$item['obd_id']]['current_oil'] = $item['current_oil'];
//                        }
//                        $mileage[$item['obd_id']]['current_mileage'] = $item['current_mileage'];
