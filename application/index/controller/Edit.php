...
class Edit extends check
{
    ...
    public function delCar()
    {
        ...
        $sid = $this->sid;
        if (!empty($id)) {
            $res = Db::execute("delete from `t_car_info` where  `id` = '$id' and `merchant_id` in($sid)");
            if (!empty($res)) {
                Db::query("delete from `t_mileage_center` where `car_id` = '$id'  and `merchant_id` in($sid)");
    ...
    public function driverInfo()
    {
        ...
        if ($rst) {
            ...
            $driver['merchant_id'] = session('uid');
    ...
    public function delDriver()
    {
        ...
        $sid = $this->sid;
        if (!empty($id)) {
            $res = Db::execute("delete from `t_driver_info` where  `id` = '$id' and `merchant_id` in($sid)");
            if (!empty($res)) {
                exit(json_encode(['error' => 0, 'msg' => '删除成功']));
            } else {
                exit(json_encode(['error' => 1, 'msg' => '数据不存在，请刷新重试']));
